...
const SessionList = (props) => {
  ...
  const [isModalDragVisible, setIsModalDragVisible] = useState(false);
  const [items, setItems] = useState([]);

  const showModalDrag = () => {
    setIsModalDragVisible(true);
  };

  const handleOk = async (updatedOrderValues) => {
    // Handle the updated order values here (updatedOrderValues)
    console.log(updatedOrderValues);
    const endpoint = `${getBaseUrl()}/tests/updateOrder`;
    try {
      toastr.clear();
      setLoading(true);
      ...
      setIsModalDragVisible(false);
      setLoading(false);
    }
    setIsModalDragVisible(false);
  };

  const handleDargCancel = () => {
    setIsModalDragVisible(false);
  };
  ...
  const handleCreate = async (values) => {
    ...
    formData.append("includeDistance", values.distanceTest || false);
    formData.append("includeTime", values.includeTimeTest || false);
    formData.append("isPractice", values.option === "practice");

    formData.append("isMaxthreshold",values?.isMaxthreshold);
  ...
  const menu = (testId) => {
    return (
      <Menu>
        ...
        <Menu.Item key="navigate" onClick={() => navigateToTest(testId)}>Athlete Assignment</Menu.Item>
      </Menu>
    );
  }
  const navigateToTest = (testId) => {
    navigate(`/assignment/test/${testId}`);
  };
      ...
      <div className="container">
        <div className="row">
          <div className="col-md-12" style={{ marginTop: "30px" }}>
            ...
            { isAdmin && <Button onClick={showModalDrag}>Re-order</Button>}
            <ReorderModal
              visible={isModalDragVisible}
              items={items}
              onOk={handleOk}
              onCancel={handleDargCancel}
            />
                    ...
                    >
                      <Card
                        ...
                        actions={[
                          ...
                          <>
                            <div key="edit" onClick={() => {
                              ...
                              showDeleteConfirm(test._id)
                            }} style={{
                              textAlign: 'center',
                              color: isAdmin ? 'inherit' : 'gray', // Conditionally apply gray color
                        ...
                        title={[
                          ...
                          >
                            <span style={{ maxWidth: "250px", overflow: "hidden" }} title={test.name}>{test.name}</span>
                            {isAdmin && (
                              <Dropdown overlay={() => menu(test._id)} trigger={["click"]}>
                                <svg
                      ...
                      >
                        ...
                        <p style={{ minHeight: "50px", wordBreak: "break-word", marginTop: "2px" }}>
                          {test.title}
                        </p>
                        <video
                          controls
